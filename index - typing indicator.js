...
const io = new Server(server, { cors: { origin: `http://localhost:3000` }, maxHttpBufferSize: 4e6 });
io.use(async (socket, next) => {
	...
	// Add user to indicator array 
	socket.on('typing-indicator-sender',(data) => {
		socket.join(data.chatid);
		socket.to(data.chatid).emit('typing-indicator-receive', data);
	})

	//remove user from indicator array
	socket.on('remove-user-from-indicator-array',(data)=>{
		socket.join(data.chatid);
		socket.to(data.chatid).emit('remove-user-from-indicator-receive', data);
	})
	...
	socket.on("send_message", async (data) => {
		...
		if(data.file_data.name !== undefined && data.file_data.type !== undefined){	
			...
			const FOLDER_PATH = 'message';
			const PRODUCT_UPLOAD_PATH = FOLDER_PATH + '/' + fileName;
			let params = { Bucket: process.env.AWS_S3_BUCKET_NAME, Key: PRODUCT_UPLOAD_PATH, Body: data.file_data.buffer };
